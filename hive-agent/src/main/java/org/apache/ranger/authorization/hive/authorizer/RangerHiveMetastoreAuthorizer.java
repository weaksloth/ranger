    ...
    public void onEvent(PreEventContext context)
        ...
        try {
            ...
            if (ss != null) {
                ...
                groups = Sets.newHashSet(ss.getAuthenticator().getGroupNames());
            } else {
                UserGroupInformation userGroupInformation = getCurrentUser();
                LOG.debug("Got current username  " + userGroupInformation);
                user = userGroupInformation.getUserName();
                groups = Sets.newHashSet(userGroupInformation.getGroups());
            }
...
//                    break;
    ...
    private Set<String> getCurrentRolesForUser(String user, Set<String> groups) {
        if (LOG.isDebugEnabled()) {
            LOG.debug("==> RangerHiveMetaStoreAuthorizer.getCurrentRolesForUser()");
        }

        Set<String>  ret  = hivePlugin.getRolesFromUserAndGroups(user, groups);

        if (LOG.isDebugEnabled()) {
            LOG.debug("<== RangerHiveMetaStoreAuthorizer.getCurrentRolesForUser() User: " + user + ", User Roles: " + ret);
        }
